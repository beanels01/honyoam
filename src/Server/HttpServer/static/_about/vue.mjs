import homepageLike from    '../_homepageLike.mjs'
let aMain={
    components:{
        hlFooter:   homepageLike.footer,
        hlMenu:     homepageLike.menu,
        hlHeader:   homepageLike.header,
        floatBall:  homepageLike.floatBall,
    },
    computed:{
        href(){
            return homepageLike.langToPath(this.currentLanguage)
        },
    },
    data:()=>({
        menu:0,
    }),
    props:['language','currentLanguage','mainSeminar'],
    template:`
        <div id=main>
            <hlHeader
                shadow=1
                :language=language
                :currentLanguage=currentLanguage
                :mainSeminar=mainSeminar
                v-model=menu
            ></hlHeader>
            <template v-if=!menu>
                <div class=a>
                    <div class=n>
                        <img src=/_about/img/logo.png>
                    </div>
                    <div class=o>
                        <div class=a>
                            <ul>
                            <li>成立於 2013 年</li>
                            <li>總公司本葉国際資産管理株式会社</li>
                            <li>位於東京港區、台北松江路設有辦公室</li>
                            <li>業務內容:
                                <ul>
                                <li>不動產預售代銷</li>
                                <li>中古屋仲介買賣、投資諮詢</li>
                                <li>不動產租賃管理一站式服務</li>
                                </ul>
                            </li>
                            </ul>
                            <img src=/_about/img/a0.png style=width:20%>
                            <img src=/_about/img/a1.png style=width:70%>
                        </div>
                        <div class=b>
                            <div class=n>
                                <div class=a>
                                    <div class=n>
                                        東京辦公室
                                    </div>
                                    <div class=n>
                                        本葉国際資產管理株式会社
                                        〒105-0012　東京都港区芝大門2‐4‐1　IZUMIビル4階
                                        TEL 03－6809－1836
                                    </div>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        台北辦公室
                                    </div>
                                    <div class=o>
                                        本葉國際資產管理股份有限公司<br>
                                        104台北市中山區松江路122號11樓<br>
                                        Tel：02-2785-5865
                                    </div>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        代表取締役社長
                                    </div>
                                    <div class=p>
                                        林彦宏
                                    </div>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        設立年月日
                                    </div>
                                    <div class=o>
                                        2013年12月19日
                                    </div>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        營業執照
                                    </div>
                                    <div class=p>
                                        東京都知事（1）第96199號
                                    </div>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        所屬團體
                                    </div>
                                    <div>
                                        <div class=o>
                                            公益社団法人東京都宅地建物取引業協会
                                        </div>
                                        <div class=p>
                                            公益社団法人全国宅地建物取引業保証協会
                                        </div>
                                    </div>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        公司網頁
                                    </div>
                                    <div class=o>
                                        www.honyoam.com
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class=p>
                        <div class=a>
                            <img src=/_about/img/b0.png><br>
                            預售案免服務費<br>
                            與日本當地同步優先選購
                        </div>
                        <div class=a>
                            <img src=/_about/img/b1.png><br>
                            大中華圈30年<br>
                            +<br>
                            日本在地28年<br>
                            不動產經驗
                        </div>
                        <div class=a>
                            <img src=/_about/img/b2.png><br>
                            一站式服務<br>
                            租賃及居家管理<br>
                            提供中、英、日語服務
                        </div>
                    </div>
                    <div class=q>
                        <p>
                            <span style=font-weight:bold>本葉概要</span>: 本葉国際資産管理株式会社在2013年12月成立於日本東京港區,專營日本不動產投資及資產管理顧問,有別於其他仲介公司, 除了中古屋買賣外並與三菱地所RESIDENCE 合作, 提供 The Parkhouse 系列預售屋,我們的預售屋價格與日本同步,且預售屋服務完全不需負擔服務費, 讓您在第一時間取得販售資料與挑選最好物件的機會。
                        </p>
                        <p>
                            我們的團隊中有日本人也有長住日本的台灣人,除了擁有中日兩地不動產證照及經驗豐富的專業人員, 不但可以日文、中文應對, 台語、英語也沒問題, 為了滿足在台客戶的需求及提供即時的服務, 同時成立了本葉國際資產管理股份有限公司作為台北的據點。
                        </p>
                        <p>
                            我們的社長擁有台灣不動產20年<span style=color:red>與在日近10年的經驗</span>, 我們的部長在日本大京穴吹不動產服務20年並且駐台4年, 兩位融合中日兩地近30年之不動產專業知識與經驗,並以培育優秀的年輕人才為目標帶領本葉團隊一起努力落實為客戶提供一個安全安心又順心的不動產投資環境。
                        </p>
                        <p>
                            我們認為更多的服務從成交後開始, 不管是協助您在日本生活上各方面的疑慮或是幫助您融入當地的生活與民情, 或是提供您不在日本期間的好管家代管服務, 本葉國際都可為您量身訂做從買屋、賣屋到租賃管理的一站式全方位服務 。
                        </p>
                    </div>
                    <div class=r>
                    </div>
                    <div class=s>
                        <span style=font-weight:bold>社長介紹</span>：代表取締役社長 林彦宏
                        半百跳脫舒適圈, 人生有4分之1待在信義房屋的林彥宏，並沒有選擇在「信義」這棟大樹底下退休，而是在50歲前離開信義，創業成立屬於自己的公司。從日本信義到自行創業 「本葉國際資產管理株式會社」，林彥宏銷售日本不動產給華人<span style=color:red;>已近10年</span>，融合中日不動產經驗近30年, 非常了解兩地文化的異同, 可以說是建立中日兩地房地產投資平台的先鋒。 擁有國際不動產(CCIM) 分析師證照及不動產經紀人執照, 是一個真正想走出台灣,立足亞洲,放眼全世界的房地產專業經理人!
                    </div>
                    <div class=t>
                        <div style="color:red;font-size:40px;">我們的服務流程與內容</div>
                        <div class=a>
                            <div>
                                <div class=a>
                                    <div class=n>
                                        STEP1<br>
                                        諮詢溝通<br>
                                        介紹優質物件
                                    </div>
                                    <div class=o>
                                        <img src=/_about/img/c0.png>
                                    </div>
                                    <div class=p>
                                        <ul>
                                        <li>諮詢與溝通了解需求</li>
                                        <li>介紹優質物件</li>
                                        <li>赴日看屋行程確定</li>
                                        </ul>
                                    </div>
                                </div>
                                <div class=b>
                                    <img src=/_about/img/arrow.png>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        STEP2<br>
                                        合約審核與簽約
                                    </div>
                                    <div class=o>
                                        <img src=/_about/img/c1.png>
                                    </div>
                                    <div class=p>
                                        <ul>
                                        <li>簽約前屋況確認</li>
                                        <li>合約書審核後簽約</li>
                                        </ul>
                                    </div>
                                </div>
                                <div class=b>
                                    <img src=/_about/img/arrow.png>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        STEP3<br>
                                        交屋
                                    </div>
                                    <div class=o>
                                        <img src=/_about/img/c2.png>
                                    </div>
                                    <div class=p>
                                        <ul>
                                        <li>協助交屋與產權登記手續</li>
                                        <li>確認出租或自住的相關事宜</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            <div>
                                <div class=a>
                                    <div class=n>
                                        STEP4<br>
                                        後續服務
                                    </div>
                                    <div class=o>
                                        <img src=/_about/img/c3.png>
                                    </div>
                                    <div class=p>
                                        <ul>
                                        <li>Second House 的好管家代管服務,house keeping 選購家具等</li>
                                        <li>納稅管理人的服務,解決您稅務雜項</li>
                                        <li>租賃管理服務並協助報稅</li>
                                        </ul>
                                    </div>
                                </div>
                                <div class=b>
                                    <img src=/_about/img/arrow.png>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        STEP5<br>
                                        其他服務
                                    </div>
                                    <div class=o>
                                        <img src=/_about/img/c4.png>
                                    </div>
                                    <div class=p>
                                        <ul>
                                        <li>依客戶需求成立日本現地法人公司</li>
                                        <li>協助取得投資簽證</li>
                                        <li>現地法人公司代管理代報稅及納稅</li>
                                        <li>台灣公司產品協助進駐日本</li>
                                        </ul>
                                    </div>
                                </div>
                                <div class=b>
                                    <img src=/_about/img/arrow.png>
                                </div>
                                <div class=a>
                                    <div class=n>
                                        STEP6<br>
                                        投資再販售
                                    </div>
                                    <div class=o>
                                        <img src=/_about/img/c5.png>
                                    </div>
                                    <div class=p>
                                        <ul>
                                        <li>市場價格查定</li>
                                        <li>協助在好的時機販售</li>
                                        <li>資金收回相關協助</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class=u>
                        <div style="color:red;font-size:30px;">* 我們的簽約合作夥伴</div>
                        <img src=/_about/img/d0.png>
                        <img src=/_about/img/d1.gif>
                    </div>
                    <div class=v>
                        <div style="color:red;font-size:30px;">* 有實績的合作夥伴</div>
                        <img src=/_about/img/e0.png>
                        <img src=/_about/img/e1.png>
                        <img src=/_about/img/e2.png>
                        <img src=/_about/img/e3.png>
                        <img src=/_about/img/e4.png>
                        <img src=/_about/img/e5.png>
                        <img src=/_about/img/e6.png>
                    </div>
                    <div class=w>
                         <div style="color:red;font-size:30px;font-weight:bold">* 合作銀行（貸款相關）</div>
                        <img src=/_about/img/f0.jpg>
                        <img src=/_about/img/f1.png>
                        <img src=/_about/img/f2.png>
                        <img src=/_about/img/f3.png>
                        <img src=/_about/img/f4.png>
                        <img src=/_about/img/f5.png>
                        <img src=/_about/img/f6.gif>
                    </div>
                </div>
                <hlFooter
                    :language=language.homepageLike.footer
                ></hlFooter>
                <floatBall
                    value=立即聯絡
                    :href=href.qaForm
                ></floatBall>
            </template>
            <hlMenu
                v-if=menu
                :language=language
                :currentLanguage=currentLanguage
                :mainSeminar=mainSeminar
            ></hlMenu>
        </div>
    `,
}
export default{
    components:{aMain},
    template:`
        <aMain
            :language=language
            :currentLanguage=currentLanguage
            :mainSeminar=mainSeminar
        ></aMain>
    `
}
